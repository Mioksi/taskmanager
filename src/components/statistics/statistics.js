...
const getTaskByDateRange = (task, dateFrom, dateTo) => {
...
const getTasksByDateRange = (tasks, dateFrom, dateTo) => tasks.filter((task) => {
  return getTaskByDateRange(task, dateFrom, dateTo);
});
...
const calcUniqCountColor = (tasks, color) => tasks.filter((task) => task.color === color).length;
